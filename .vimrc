...
augroup basic_abbreviations
	...
	autocmd FileType ruby iabbrev <buffer> iffe if<CR>else<CR>end<Up><Up>
...
augroup erb_auto_key
	autocmd!
	autocmd FileType eruby nnoremap <buffer> <localleader>r I<%<Space><Esc>A<Space>%><Esc>
	" when the line should be present in html 
	autocmd FileType eruby nnoremap <buffer> <localleader>pr I<%=<Space><Esc>A<Space>%><Esc>
	autocmd FileType eruby nnoremap <buffer> <localleader>ur I<Del><Del><Del><Del><Esc>A<BS><BS><BS><Esc>
augroup END
" }}}
